...
export const WEEKLY_STUDY_GOAL_MINUTES = DAILY_STUDY_GOAL_MINUTES * 7;
...
export function buildWeekUsage(
  ...
  fallback?: { minutes: number; dateKey: string | null },
): WeekDayMinutes[] {
  ...
  const hasAny = buckets.some((bucket) => bucket.minutes > 0);
  if (!hasAny && fallback?.minutes && fallback.dateKey && index.has(fallback.dateKey)) {
    index.get(fallback.dateKey)!.minutes = fallback.minutes;
  }
