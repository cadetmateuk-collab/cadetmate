...
export async function addXP(
  userId: string,
  gained: number,
  opts?: { reviews?: number; timeSec?: number; notify?: boolean },
  ...
  // Keep platform gamification in sync (canonical total — do not double-count on dashboards)
  const { awardXP } = await import('@/lib/gamification');
  await awardXP(
    supabase,
    userId,
    gained,
    opts?.notify === false || gained <= 0 ? undefined : 'Flashcard study',
    { absoluteTotal: xp },
  );
