...
/** If the user has been away a long time, soften ease/intervals so reviews resume sooner. */
    ...
    case 'exam_cram': {
      const weak = [...pool].sort(
        (a, b) =>
          ...
          a.progress.ease_factor - b.progress.ease_factor,
      );
      return weak[0];
    }
    case 'smart_review': {
      ...
      if (due.length === 0)
        return [...pool].sort(
          (a, b) =>
            new Date(a.progress.next_review).getTime() -
            new Date(b.progress.next_review).getTime(),
        )[0];
